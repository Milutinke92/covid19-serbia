...
df_death_cases = pd.read_json("data_death_cases.json")
...
df_death_cases['time'] = df_death_cases['time'].apply(lambda x: datetime.strptime(x, "%Y-%m-%d"))
...
ax.bar(df_death_cases['time'].array, df_death_cases['num_of_cases'].array, width, label='death cases')
